    ...
    <ul>
         <li>
              <% Dial4Jobz.Models.Job job=new Dial4Jobz.Models.Job(); %>
                 ...
                 <%: Html.ActionLink("DashBoard", "ConsultantDashBoard", "ConsultantReport", new { consultantId = Model.Id }, new { target = "_blank" })%>  | <br />

                 <%: Html.ActionLink("Send Account Details", "AccountDetailsEmployer", "AdminHome", new { organizationId= 0, consultantId = Model.Id }, new { title = "Account Details" })%> | <br />
             
                 <%if(Model.Email!=null) { %>
                    <%: Html.ActionLink("Send Vas Details", "SendVasDetailsToEmployer", "AdminHome", new { organizationId=0, consultantId = Model.Id }, new { title = "Send Vas Details" })%> <br />
                 <%} %><br />
