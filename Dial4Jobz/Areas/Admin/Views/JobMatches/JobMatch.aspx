...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
	Job Matches
    <%Session["candidId"] = Request.QueryString["Id"]; %>
</asp:Content>
...
<asp:Content ID="Content3" ContentPlaceHolderID="ScriptContent" runat="server">
    ...
    function Pagination(pageNo) {debugger;
        $("#PageNo").val(pageNo);
        $("#PagingForm").submit();
    }
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
   ...
   <% Dial4Jobz.Models.Repositories.Repository _repository = new Dial4Jobz.Models.Repositories.Repository(); %>
       ...
       <% HttpCookie aCookie = Request.Cookies["candidateId"];%>
    <% if (aCookie != null)
       {
           Session["candidateId"] = aCookie.Value;
       }%>

     <% var candidateId = ViewData["CandidateId"].ToString();%>

     <% var candidate = _repository.GetCandidate(Convert.ToInt32(candidateId)); %>
   

    <%:Html.ActionLink("Add New Candidate","AddCandidate","AdminHome") %> |  <%: Html.ActionLink("Candidate DashBoard", "CandidateDashBoard", "AdminHome", new { candidateId = candidateId }, new { target = "_blank" })%> | <%:Html.ActionLink("Send Account Details", "AccountDetails", "AdminHome", new { candidateId = candidateId }, new { target = "_blank" })%>
    <br />


     <h3><center>Direct Verification</center></h3>
    ...
    <script type="text/javascript">
        ...
        $("#directverification").click(function () {
            ...
            $('#verifymobile').text('Direct Verification')
                .attr('href', 'GetCandidateMobileNumber?DirectVerification=' + '<%= ViewData["CandidateId"] %>');
        });

        $(document).ready(function () {
            $('#directverification').click(function () {
                var name = '<%= Session["candidateId"] %>';
                var data = 'id=' + name;
                $.ajax({
                    type: "GET",
                    url: '/Admin/JobMatches/DirectVerification',
                    data: data,
                    dataType: 'json',
 ...
 <% Html.BeginForm("Send", "JobMatches", FormMethod.Post, new { @id = "Send" });  %>


   <% if (!String.IsNullOrEmpty(candidate.ResumeFileName))
             { %>
                <p>
                    <a href="<%:Url.Action("DownloadResumeFromJobMatches","JobMatches",new { fileName=candidate.ResumeFileName}) %>"><img src="../../../../Content/Images/Download_resume.jpg" height="30px" width="95px" /></a>
                </p>
         <% } %>
...
<asp:Content ID="Content5" ContentPlaceHolderID="SideContent" runat="server">
    ...
    <h3>Send Plan Details By SMS</h3><br />

    <div class="vastext">
        <%: Html.ActionLink("Spot Interview", "SendTemplates", "JobMatches", new { planName = "SI" }, new { title = "SI Details" })%> <br />
        <%: Html.ActionLink("Job Alert", "SendTemplates", "JobMatches", new { planName = "JobAlert" }, new { title = "RAJ Details" })%><br />
        <%: Html.ActionLink("Display Resume", "SendTemplates", "JobMatches", new { planName = "Dpr" }, new { title = "DPR Details" })%><br />
    </div>
